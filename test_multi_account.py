...
assert len(accounts) == 2, f'Expected exactly 2 broker accounts, got {len(accounts)}'
for acc in accounts:
...
print('\n=== 4. Testing Inspection on Account 2 (Real Account) ===')
acc2 = am.get_account_by_id('2')
text2, markup2 = inspect_account_trades(acc2)
assert f'ACCOUNT #2: {acc2.name.upper()}' in text2
...
# Restore back to Account 2
am.set_active_account('2')
assert am.get_active_account().id == '2'
print('Restored to Account 2')

print('\n[SUCCESS] ALL MULTI-ACCOUNT TESTS PASSED!')
