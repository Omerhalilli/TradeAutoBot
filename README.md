...
* **👥 Multi-Account & Profile Switcher**: `/accounts` or `/switch` displays an interactive inline panel to switch between **Invest-AZ Demo** (Account `1234567`) and **Invest-AZ Real** live trading. Instantly inspects whether the selected account has active **BUY or SELL** functions running with ticket-level diagnostics, lot volume, floating P/L, and detects if terminal login matches target account mode.
