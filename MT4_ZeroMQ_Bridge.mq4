...
string HandleCloseAll()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      bool ok = OrderClose(OrderTicket(), OrderLots(), closePrice, InpSlippage, clrOrangeRed);
      if(ok)
      {
         closed++;
         realizedPL += pl;
      ...
      {
         failed++;
         PrintFormat("[ZMQ Bridge] Failed to close #%d %s: err=%d", OrderTicket(), OrderSymbol(), GetLastError());
      }
   }
...
int OnInit()
{
   ...
   DebugLog("Socket bound successfully to " + InpBindAddress); g_isZmqReady = true;
      bool msTimerOk = EventSetMillisecondTimer(100);
...
void OnTimer()
{
   ...
   int bytesRecv = zmq_recv(g_socket.ref(), reqBuf, 4096, 1); // 1 = ZMQ_DONTWAIT
   
   if(s_ticks % 20 == 0)
   {
      DebugLog("Tick " + IntegerToString(s_ticks) + " recv=" + IntegerToString(bytesRecv) + " errno=" + IntegerToString(zmq_errno()));
   }
